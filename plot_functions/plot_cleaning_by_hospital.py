...
def plot_cleaning_by_hospital(elementos_df, aseos_df, time_aggregation='monthly', graph_type='Bar Chart', show_numbers=False, color_palette='Plotly'):
    ...
    if 'CENTRO HOSP.' not in elementos_df.columns or 'CENTRO HOSP.' not in aseos_df.columns:
        return "<p>Error: 'CENTRO HOSP.' column not found in data.</p>"
    ...
    elementos_grouped = elementos_df.groupby(['Period', 'CENTRO HOSP.'])['mean_cleanliness'].mean().reset_index()
    aseos_grouped = aseos_df.groupby(['Period', 'CENTRO HOSP.'])['mean_cleanliness'].mean().reset_index()
    ...
    if graph_type == 'Bar Chart':
        fig_elementos = px.bar(
            ...
            color='CENTRO HOSP.',
            barmode='group',
            title='Mean Cleanliness per Hospital (Elementos)',
        ...
        fig_aseos = px.bar(
            ...
            color='CENTRO HOSP.',
            barmode='group',
            title='Mean Cleanliness per Hospital (Aseos)',
            text='mean_cleanliness' if show_numbers else None,
            color_discrete_sequence=colors
        )
